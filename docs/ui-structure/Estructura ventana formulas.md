...
└── root (QVBoxLayout, transparente, sin borde, márgenes 14 px, separación 10 px)
    ...
    └── splitter horizontal (objectName `recipesMainSplitter`, fondo transparente, sin borde, handle transparente de 5 px, childrenCollapsible=False, tamaños iniciales 332 / 930)
        ...
        │           │   ├── customer_filter_input (QLineEdit, objectName `customerRecipeFilterInput`, fondo #FFFFFF, borde #C8D2DF, radio 6 px)
        │           │   └── customer_filter_clear_btn (QPushButton, objectName `customerRecipeFilterClearButton`, icono `close-white.svg`, fondo rojo #D96464, borde #D96464, 32 x 32 px; deshabilitado si el campo está vacío)
