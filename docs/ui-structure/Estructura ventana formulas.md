...
- `Exportar PDF` exige una receta guardada y permite elegir diseño simple, extendido o mínimo. El formato `Mínimo` abre una vista previa, permite seleccionar mediante radios excluyentes si se incluye el escandallo y muestra una copia de la tabla Receta y, opcionalmente, de la tabla Escandallo; el título es el nombre de la receta. `Imprimir` y `Exportar Excel` muestran actualmente un aviso de fase futura.
