...
- Fusionar cliente: acción del menú contextual de `customersListTable`; abre `customerMergeDialog` con cliente origen bloqueado, filtro por ocurrencia para localizar el cliente destino, selector de cliente destino filtrado, resumen de dependencias a mover y confirmación final antes de ejecutar `CustomerService.merge_customers()`.
