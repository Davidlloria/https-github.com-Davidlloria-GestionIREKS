...
class SettingsOrdersImportView:
    section_info_label: str = "Importación de pedidos (JSON)"
...
class SettingsOrdersImportService:
    ...
    def import_orders_json(self, source: Path, almacen_id: str) -> SettingsOrdersImportOutcome:
        ...
        if clean_source.suffix.lower() != ".json":
            raise ValueError("El archivo seleccionado debe tener extensión .json.")
        ...
        summary = [
            "Pedido importado: (sin número)",
            f"Líneas importadas: {result.imported_items}",
            f"Líneas con código inexistente: {len(result.skipped_unknown)}",
            f"Líneas inválidas: {result.skipped_invalid}",
        ]
        if unknown_unique:
            summary.append(f"Códigos no encontrados: {unknown_preview}{unknown_extra}")
        return SettingsOrdersImportOutcome(
            ok=True,
            message="Importación de pedidos completada.",
            log_message=f"Importación de pedidos OK: {clean_source.name} | líneas={result.imported_items}",
