...
class SettingsSalesPreviewView:
    ...
    workbook_import_error_title: str = "Importación de libro IGSA"
...
class SettingsSalesPreviewService:
    ...
    def preview_igsa_pdf_files(self, file_paths: list[Path]) -> SettingsSalesPdfPreviewOutcome:
        ...
        if not lines:
            detail = "\n".join(errors[:10]) if errors else "No se pudieron extraer líneas."
    ...
    def preview_igsa_workbook(self, file_path: Path) -> SettingsSalesWorkbookPreviewOutcome:
        ...
        if not lines:
            detail = "\n".join(parse_errors[:20]) if parse_errors else "No se pudieron extraer líneas."
            raise ValueError(detail)
    ...
    def _validate_paths(self, file_paths: list[Path], *, allowed_suffixes: set[str]) -> list[Path]:
        ...
        if not clean_paths:
            raise ValueError("Selecciona al menos un archivo válido.")
        return clean_paths
