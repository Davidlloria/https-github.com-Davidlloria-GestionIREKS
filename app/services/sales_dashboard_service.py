...
class SalesDashboardService:
    ...
    def load_snapshot(self, *, today: date | None = None) -> SalesDashboardSnapshot:
        ...
        years = self.annual_sales_service.list_years_clientes()
        ...
        for raw_row in raw_rows:
            ...
            bucket[key] = float(bucket.get(key, 0.0) or 0.0) + float(getattr(raw_row, "kg", 0.0) or 0.0)
        ...
        customer_drop_rows.sort(
            key=lambda row: (row.delta_kg, row.kg_curr, row.cliente_nombre.casefold(), row.cliente_codigo.casefold())
        )

        zero_consumption_rows = [row for row in customer_rows if row.kg_prev > 1e-9 and abs(row.kg_curr) <= 1e-9]
        zero_consumption_rows.sort(
            key=lambda row: (-row.kg_prev, row.cliente_nombre.casefold(), row.cliente_codigo.casefold())
        )

        island_buckets: dict[str, dict[str, object]] = defaultdict(
            lambda: {"customers": set(), "kg_prev": 0.0, "kg_curr": 0.0}
        )
        type_buckets: dict[str, dict[str, object]] = defaultdict(
            lambda: {"customers": set(), "kg_prev": 0.0, "kg_curr": 0.0}
        )
